...
Helpers depends on app models.
If a function does not depends on app models, place it in utils.py
"""
...
from ..azure_parts.helpers import batch_upload_parts_to_customvision
...
from ..images.helpers import upload_images_to_customvision_helper
...
def pull_cv_project_helper(project_id, customvision_project_id: str, is_partial: bool):
    ...
    # Check
    if not project_obj.is_pullable(customvision_id=customvision_project_id):
        return
    ...
    while img_index <= imgs_count:
        ...
        for img in imgs:
            logger.info("*** image %s", img_counter)
            for region in img.regions:
                ...
                if created:
                    logger.info("Downloading image %s.", img.id)
                    img_obj.get_remote_image()
                    logger.info("Setting image %s label.", img.id)
                    img_obj.set_labels(
                        left=region.left,
                        top=region.top,
                        width=region.width,
